...
- **Point résiduel à surveiller, pas corrigé ici** : sur cette transaction de test, le frais Stripe réellement prélevé sur la plateforme s'est révélé plus élevé (90 centimes) que l'hypothèse `TAUX_STRIPE = 0.015` + frais fixe utilisée par `computeMontantOrganisateurCents` pour estimer `application_fee_amount` (77 centimes envoyés, contre 90 réellement prélevés par Stripe côté plateforme) — la plateforme reste donc légèrement déficitaire sur cette transaction précise (-13 centimes) même après le correctif, alors que l'organisateur, lui, reçoit bien pile le montant promis. Cause possible non confirmée : le taux réel appliqué par Stripe pour une destination charge via Connect peut différer de l'hypothèse simple carte UE (1,5 % + 0,25 €) posée dans `lib/fee-calculation.ts`, ou la carte de test utilisée (`4242 4242 4242 4242`) simule un profil de frais spécifique. Sujet financier — à valider avec l'utilisateur avant d'ajuster `TAUX_STRIPE`, pas tranché ici.

À ne pas passer en clés Stripe live avant d'avoir statué sur ce dernier point.
