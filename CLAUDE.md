...
- `AjouterArticleForm` réorganisé en deux onglets (Par lien / Saisie manuelle) — le lien produit reste obligatoire dans les deux onglets (`gift_items.source_url` est `not null` en base, contrairement à la maquette qui n'a pas de champ lien dans l'onglet manuel).
...
- Point laissé de côté délibérément : le bouton "Réglages de la liste" de la maquette n'a pas de comportement défini (ni dans le mock, ni ailleurs dans le produit) — non repris. Le badge de statut et le bouton "Ouvrir ma liste aux invités"/"Repasser en brouillon" (déjà actés dans "Statut de liste") ont été conservés dans la bannière à la place, la maquette ne les représentait pas mais rien n'indique que cette notion soit abandonnée.
