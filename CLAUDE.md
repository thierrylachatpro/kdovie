...
**Testé** : `tsc`/`lint`/`build` propres. Rendu vérifié par capture d'écran avec des données bouchon (liste, édition de pseudo, confirmation de désactivation) — conforme. **Pas testé en conditions réelles** : bloqué tant que la migration `0019` n'est pas appliquée à la base distante (comme les migrations précédentes en attente) ; le blocage de connexion dans le callback n'a donc pas non plus été vérifié de bout en bout.
