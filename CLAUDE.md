...
- `transfer_data.destination` **+ `on_behalf_of`** vers le compte Connect de l'organisateur, et `application_fee_amount` (commission Kdovie 1 %), posés côté serveur (`app/liste/[slug]/contribution-actions.ts`) — le `on_behalf_of` est nécessaire pour que les frais de traitement Stripe soient portés par l'organisateur et non par la plateforme (sinon Stripe les prélève par défaut sur le solde de la plateforme, ce qui contredirait "les frais Stripe ne sont pas absorbés par Kdovie").
