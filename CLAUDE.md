...
- **Déclencheur affiné par rapport au cadrage initial** : ne compte que les articles avec `status != 'reserve'` en plus du mode `auto`/`cotisation_obligatoire` et de la liste ouverte — une fois qu'un article est verrouillé en réservation directe, l'option cotiser est masquée pour tous les invités (voir "Règle de gestion : réservation vs cotisation par article"), donc aucun risque de rupture sur cet article précis même si son `mode` dit encore `auto`. Sans ce filtre, le bandeau se serait affiché à tort pour des listes où toutes les cotisations potentielles sont déjà devenues impossibles.
