...
- **Testé** : `tsc`/`lint`/`build` propres. Comportement de la case vérifié réellement (Playwright, page de prévisualisation temporaire supprimée ensuite) : non précochée au chargement, message bloquant affiché si on tente de payer sans cocher, message levé une fois cochée. Validation serveur vérifiée par inspection (garde de 3 lignes en tête de fonction) — **pas exercée en conditions réelles** (nécessiterait un organisateur de test avec compte Stripe Connect actif).
