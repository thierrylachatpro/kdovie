...
- **Testé** : build/tsc/lint propres ; rendu visuel vérifié par capture d'écran pour les deux états de la page (introuvable, et réservation active — ce second état testé via une page de preview temporaire avec données bouchon, la migration n'étant pas encore appliquée à la base distante pour tester avec une vraie réservation) ; `truncateTitle` vérifié unitairement sur plusieurs titres réels de la base. **Pas testé** : le flux complet en conditions réelles (appel RPC réel, verrouillage/déverrouillage de l'article) — bloqué tant que la migration `0017` n'est pas appliquée manuellement, comme les migrations précédentes.
