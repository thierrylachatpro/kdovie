...
- **Projet Supabase de dev créé** (19 août 2026) : `kdovie-dev`, ref `hvyinuoebkzghrbcbnqa` (à distinguer du ref de prod `ppsaiaesnvwnkzjisvdr`, cité en toutes lettres ici uniquement comme repère — mots de passe/clés jamais consignés dans ce fichier, versionné). Les 18 migrations ont été rejouées dessus via `supabase db push --db-url ...` et vérifiées (`supabase migration list` confirme `local` = `remote` sur toute la ligne).
- **Reste à faire côté utilisateur** (aucune dépendance technique bloquante côté code) : dans le dashboard Vercel — Project Settings → Environments → Production → Branch Tracking : changer la branche de production pour `dev` (le réglage a été déplacé par Vercel depuis Project Settings → Git, qui ne le montre plus) ; Project Settings → Domains : éditer `kdovie.com` pour l'épingler sur la branche `main` ; Project Settings → Environment Variables : poser les identifiants de la nouvelle base de dev (`NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`) sous le scope "Production", et ajouter un override "Preview" restreint à la branche `main` avec les identifiants de la vraie base de prod pour ces 3 mêmes variables.
