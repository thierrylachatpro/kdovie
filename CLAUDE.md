...
**Reversement automatique remplacé par un déclenchement manuel de l'organisateur (6 septembre 2026)** : le calendrier de virement par défaut décrit ci-dessus (automatique, quotidien) est remplacé par un virement déclenché explicitement par l'organisateur, article par article — **implémenté**, voir section dédiée "Reversement manuel de la cagnotte par article, initié par l'organisateur" plus bas pour le détail et les actions utilisateur restantes (migration `0025`, script de bascule des comptes existants, `CRON_SECRET`).
...
- **Droit de rétractation : tranché le 6 septembre 2026, voir section dédiée "Droit de rétractation : renonciation expresse à la cotisation" plus bas** — ce n'est plus un point ouvert. La politique de remboursement, elle, était déjà actée ailleurs dans ce fichier ("pas de remboursement, pas de plafond", voir "Règle de gestion : réservation vs cotisation par article" en tête de fichier) et peut être reprise telle quelle dans les CGV.
...
## Droit de rétractation : renonciation expresse à la cotisation (6 septembre 2026)

Dernier point ouvert pour un texte de CGV complet (voir section CGV ci-dessus). Décision de l'utilisateur : **option retenue = renonciation expresse**, pas la formulation "zone grise" plus légère envisagée en alternative — nécessite un vrai développement, pas seulement un texte.

- **Base légale** : art. L221-28 du Code de la consommation ([Légifrance](https://www.legifrance.gouv.fr/codes/article_lc/LEGIARTI000044563170)) — le droit de rétractation de 14 jours ne s'applique pas à une prestation de service pleinement exécutée avant la fin du délai, à condition que trois éléments soient réunis : (1) l'invité a donné un accord exprès et préalable pour que l'exécution commence immédiatement, sans attendre la fin du délai de rétractation, (2) il a expressément renoncé à son droit de rétractation, (3) il en a été informé avant paiement. Une cotisation Kdovie s'exécute quasi immédiatement (Stripe transfère à l'organisateur dès la confirmation du paiement, `transfer_data.destination`) — l'exception est donc pertinente, mais elle doit être déclenchée explicitement par ces trois conditions, jamais supposée acquise par défaut.
- **Sans ces conditions, le droit de rétractation s'applique en principe** même si l'argent a déjà transité côté Stripe — le problème pratique déjà identifié (remboursement impossible une fois l'organisateur payé) ne fait pas disparaître le droit légal en soi, d'où la nécessité de cette case plutôt que de l'ignorer.

### Ce qui doit être développé

- **Nouvelle case à cocher, non précochée par défaut**, dans le parcours de cotisation invité (`ContributionModal`, avant le déclenchement du paiement Stripe Checkout) — texte proposé, à ajuster librement à l'implémentation : *"Je demande que ma cotisation soit exécutée immédiatement et je reconnais qu'en la validant, je renonce à mon droit de rétractation de 14 jours applicable aux achats à distance."*
- **Validation bloquante côté client ET côté serveur** avant de créer la Checkout Session (`app/liste/[slug]/contribution-actions.ts`) — même principe que les autres validations déjà en place dans ce fichier (jamais une case cochée seulement côté UI sans re-vérification serveur, cohérent avec la règle déjà appliquée ailleurs dans le produit, ex. le trigger Postgres sur `mode`/`status`).
- **Trace de cet accord à conserver** : au minimum un horodatage de l'acceptation, idéalement stocké sur la ligne `contributions` elle-même (nouvelle colonne, ex. `retractation_renoncee_at timestamptz`) plutôt que déduit implicitement de l'existence du paiement — en cas de litige, il faut pouvoir prouver que la case a bien été cochée avant paiement, pas seulement que le paiement a eu lieu. Migration à prévoir, nom exact et modalités à trancher à l'implémentation.
- **CGV** : le texte doit refléter ces trois conditions explicitement (accord exprès à l'exécution immédiate, renonciation expresse au droit de rétractation, information préalable) — pas une simple mention en passant, puisque c'est précisément ce qui permet d'invoquer l'exception légale.
- **Hors périmètre pour cette tâche** : les réservations directes (sans paiement, pas de cotisation) ne sont pas concernées par le droit de rétractation au sens de cette exception — seul le flux de cotisation via Stripe Checkout est concerné.

**Statut : implémenté et testé dans la mesure du possible (6 septembre 2026).**

- **Case à cocher** (`components/gift-items/ContributionModal.tsx`) : `<input type="checkbox">` non précochée (`useState(false)`), texte du brief repris tel quel, posée entre le champ e-mail et le bouton "Continuer vers le paiement". `handleSubmitMontant` bloque tant qu'elle n'est pas cochée (`!montantValide || !retractationRenoncee` → `setTouched(true)` + message "Vous devez cocher cette case pour pouvoir cotiser").
- **Validation bloquante serveur** (`app/liste/[slug]/contribution-actions.ts`) : `createContribution` prend un 6ᵉ paramètre `retractationRenoncee: boolean` ; si `!== true`, retour `{ error: … }` **avant** toute requête Supabase ou appel Stripe — aucune Checkout Session n'est créée. Même patron que les autres gardes de ce fichier.
- **Traçabilité** : migration `0024_contributions_retractation_renoncee.sql` — colonne `contributions.retractation_renoncee_at timestamptz` (nullable ; lignes antérieures sans valeur, toute nouvelle cotisation en a forcément une). Horodatée à `new Date().toISOString()` dans l'`insert` de la ligne `contributions`, donc posée **avant** la création de la Checkout Session. `lib/supabase/types.ts` mis à jour à la main (même pratique que les migrations précédentes en attente). **À appliquer sur dev + prod par l'utilisateur.**
- **CGV** (`app/cgv/page.tsx`) : page complète rédigée (remplace le placeholder "en cours de rédaction"). Section 6 "Droit de rétractation" détaille explicitement les trois conditions (accord exprès à l'exécution immédiate, renonciation expresse, information préalable) ; section 5 "Exécution immédiate", section 7 "Absence de remboursement" (pas de remboursement, pas de plafond, surplus acquis à l'organisateur), section 8 avec la mention transitoire du médiateur reprise mot pour mot du cadrage.
- **Testé** : `tsc`/`lint`/`build` propres. Comportement de la case vérifié réellement (Playwright, page de prévisualisation temporaire supprimée ensuite) : non précochée au chargement, message bloquant affiché si on tente de payer sans cocher, message levé une fois cochée. Validation serveur vérifiée par inspection (garde de 3 lignes en tête de fonction).
- **Migration `0024` appliquée sur dev et prod par l'utilisateur.**
- **Validé en conditions réelles le 6 septembre 2026** : l'utilisateur a effectué un vrai test de paiement (vraie cotisation, vrai organisateur avec compte Stripe Connect actif) et confirmé que tout fonctionne — paiement, page CGV, et case de renonciation au droit de rétractation. Plus aucun point ouvert sur cette fonctionnalité.

## Reversement manuel de la cagnotte par article, initié par l'organisateur (6 septembre 2026)

Aujourd'hui, l'argent cotisé transite automatiquement du solde Stripe du compte connecté vers le compte bancaire de l'organisateur, selon le calendrier par défaut de Stripe (`payout_schedule.interval = "daily"` implicite — jamais réglé explicitement dans le code) : un premier délai de 7 à 14 jours (délai de risque standard Stripe pour un nouveau compte), puis un virement quotidien automatique tant qu'il y a un solde disponible. L'organisateur ne fait rien, l'argent arrive tout seul.

**Décision de l'utilisateur : remplacer ce mécanisme par un virement déclenché explicitement par l'organisateur**, article par article — un bouton "Me reverser cette cagnotte" sur chaque cadeau en cagnotte, avec popup de confirmation précisant le montant, puis message de validation avec le délai d'arrivée prévu.

### Base technique (vérifiée sur la documentation Stripe du jour, pas supposée)

- Stripe Connect expose un réglage **par compte connecté** : `settings.payouts.schedule.interval`, valeurs possibles `daily`/`weekly`/`monthly`/`manual`. En `manual`, Stripe ne vire plus jamais automatiquement — l'argent reste sur le solde du compte connecté jusqu'à ce qu'un virement soit déclenché explicitement via l'API Payouts (`stripe.payouts.create({ amount, currency }, { stripeAccount: <id du compte connecté> })`), la plateforme agissant "pour le compte de" l'organisateur via l'en-tête `Stripe-Account`.
- **C'est un réglage global par compte, pas par cagnotte** : pas de mode hybride natif ("cette cagnotte en manuel, celle-là en automatique") — passer un organisateur en manuel s'applique à tout son solde.
- `amount` est un montant libre en centimes, mais plafonné par le solde **disponible** (`available`, pas `pending`) du compte à l'instant de l'appel — sinon erreur `insufficient_funds`. Le solde réel s'obtient via `stripe.balance.retrieve({ stripeAccount })`, qui distingue `available`/`pending`.
- **Obligation de conformité à ne pas ignorer** : en mode manuel, Stripe impose qu'un virement ait lieu au moins tous les 90 jours pour un compte français (90 jours étant le plafond "tous pays sauf USA/Thaïlande") — c'est la plateforme (Kdovie), pas Stripe, qui porte la responsabilité de ne pas dépasser ce délai. Rien n'indique dans la documentation que Stripe déclenche lui-même un virement forcé à l'échéance — c'est un point de vigilance à construire côté Kdovie, pas un filet automatique fourni par Stripe.
- Délai d'arrivée d'un virement une fois créé : standard Stripe, généralement autour de **2 jours ouvrés** (à ne pas confondre avec les "Instant Payouts", option payante distincte pour un virement en quelques minutes — hors périmètre ici, pas demandée).

### Décisions actées avec l'utilisateur

- **Bascule en manuel pour tous les comptes organisateurs**, pas un choix laissé à l'organisateur — comportement global du produit, cohérent avec le fait que le bouton de reversement devient le seul déclencheur.
- **Filet de sécurité pour l'obligation des 90 jours : un email de rappel avant l'échéance** (pas de virement automatique forcé de secours). Risque assumé explicitement : si l'organisateur ignore l'email, le risque de dépassement du délai reste réel — pas de garde-fou automatique en plus, décision consciente de l'utilisateur, à documenter comme telle (même catégorie que les autres risques déjà assumés du projet, ex. objet social/RC Pro).

### Ce qui doit être développé

- **Bascule du réglage de payout** :
  - Pour les **nouveaux comptes** : poser `payout_schedule.interval: "manual"` dans `ensureOrganizerStripeAccount` (`lib/stripe-connect-account.ts`), au moment de `stripe.accounts.create`.
  - Pour les **comptes déjà onboardés** : script ponctuel (même famille que `cleanup-organizer.mjs`) qui parcourt `organizer_stripe_accounts` et appelle `stripe.accounts.update(id, { settings: { payouts: { schedule: { interval: "manual" } } } })` sur chacun — sans ce script, seuls les nouveaux comptes créés après le déploiement seraient en manuel, les comptes existants resteraient sur l'automatique par défaut. À exécuter une fois sur dev puis sur prod, pas seulement écrit.
- **Comptabilité interne par article** (le point le plus délicat) : le solde Stripe est fongible au niveau du compte entier, pas réparti par cagnotte — Kdovie doit donc suivre lui-même combien a déjà été reversé pour chaque article. Nouvelle table `gift_item_payouts` (`gift_item_id`, `stripe_payout_id`, `amount_cents`, `created_at`) plutôt qu'une simple colonne cumulative — garde une vraie trace par virement, cohérent avec la pratique déjà en place dans ce produit de conserver un historique plutôt qu'un total opaque (ex. `contributions`). "Montant restant à reverser pour cet article" = `funded_amount_cents(article) − somme(gift_item_payouts pour cet article)`.
- **Bouton "Me reverser cette cagnotte"** sur `GiftItemCard` (page de gestion `/compte/evenements/[slug]`), visible uniquement si l'article est en `status = 'cagnotte'`, que le montant restant à reverser (calcul ci-dessus) est `> 0`, et que le compte Stripe de l'organisateur est actif.
- **Popup de confirmation**, montant calculé comme `min(montant_restant_a_reverser_pour_cet_article, solde_disponible_du_compte)` :
  - Si ce montant est inférieur au montant restant théorique (parce qu'une autre cagnotte du même organisateur a déjà puisé dans le même solde, ou qu'une partie des fonds n'est pas encore disponible/compensée par Stripe), l'expliquer clairement dans la popup — même exigence de transparence que le détail des frais déjà affiché à l'invité (voir "Cagnotte et frais") : ex. *"X € sur Y € cotisés peuvent être reversés maintenant. Le reste (Z €) est en cours de compensation et sera disponible d'ici quelques jours."*
  - Si le montant disponible est nul, ne pas proposer un clic qui échouerait — bouton désactivé avec message explicite plutôt qu'une erreur Stripe brute.
  - Bouton "Confirmer le virement" → nouvelle Server Action qui **recalcule le montant côté serveur** avant d'appeler l'API (jamais confiance dans une valeur venue du client, même principe que partout ailleurs dans ce produit), puis `stripe.payouts.create(...)`, puis insertion de la ligne `gift_item_payouts`.
- **Message de validation après le clic** : montant réellement viré + délai indicatif, formulé prudemment (pas une promesse ferme) — ex. *"Virement de X € lancé — il devrait arriver sur votre compte bancaire sous 2 jours ouvrés."*
- **Badge sur `GiftItemCard`** : montant déjà reversé sur le total cotisé (ex. "150,00 € déjà reversés sur 200,00 €"), pour que l'organisateur voie l'état sans avoir à rouvrir la popup.
- **Email de rappel (filet 90 jours)** : nouveau template email (même schéma `EmailLayout`/`emailStyles` que les emails existants), déclenché par une tâche planifiée (cron) qui repère les comptes organisateur avec un solde Stripe disponible non nul depuis plus longtemps qu'un seuil (ex. 70 jours) sans virement récent. Nécessite de connaître la date du dernier virement effectif par compte — soit via l'API Stripe (liste des payouts du compte connecté), soit via une nouvelle colonne `organizer_stripe_accounts.last_payout_at` mise à jour à chaque virement réussi (plus simple à interroger que de rappeler Stripe à chaque exécution du cron).

### Hors périmètre pour cette tâche

- Instant Payouts (option payante Stripe pour un virement en quelques minutes) — pas demandé, backlog possible si l'utilisateur veut accélérer l'arrivée des fonds plus tard.
- Virement automatique de secours au-delà de l'email de rappel — décision explicite de l'utilisateur de ne pas en construire un pour l'instant (voir "Décisions actées" ci-dessus).
- Reversement partiel à montant libre saisi par l'organisateur — pour cette première version, le bouton propose le montant calculé (plafonné par le disponible), pas un champ de saisie libre. À reconsidérer plus tard si l'utilisateur le demande.

**Statut : implémenté le 6 septembre 2026, testé dans la mesure du possible.**
...
- **UI (`GiftItemCard` via `GiftItemsList` + page)** : bouton « Me reverser cette cagnotte » (vert sauge) sur chaque article `status = 'cagnotte'` avec montant restant > 0 et compte Stripe actif. Panneau de confirmation en place (nouveau `mode = "reversing"`, désactive le glisser-déposer comme l'édition) : montant qui sera viré + explication si plafonné par le disponible (« Le reste (X €) n'est pas encore disponible… »). Après clic : message « Virement de X € lancé — sous 2 jours ouvrés ». Badge « X € déjà reversés sur Y € cotisés » sous la barre de cagnotte. Le solde Stripe est lu **une fois** par chargement de page (fongible au niveau du compte), passé à toutes les cartes.
